...
| T-02 | I, E | A backend URL points off-machine, turning the gateway into a relay that sends tool arguments to an unintended remote host. | Loopback HTTP remains the default. Remote southbound access requires explicit `policy.allow_non_loopback_backends = true` and is then constrained to exact default-port `https://<machine>.<tailnet>.ts.net/mcp` endpoints: arbitrary Internet hosts, non-TLS URLs, userinfo, query strings, fragments, custom ports, and alternate paths are rejected (`src/config/validate.rs`). | `src/config/validate.rs` `tailnet_https_backends_require_explicit_policy_and_stay_narrow`; `src/backend/registry.rs` `explicit_policy_allows_tailnet_https_backend` |
...
| T-09 | T, E | A tampered configuration widens the security posture (listener escape hatches, remote backend scope, hot reload, unknown keys sneaking past defaults). | `deny_unknown_fields` at top, `[server]`, and backend level; `schema_version` gate; `allow_non_loopback_listener` is rejected outright; `allow_non_loopback_backends` only opens the narrow Tailscale HTTPS profile from T-02; hot reload is disabled so changed policy cannot bypass startup validation. | `src/config/validate.rs`; `tests/policy.rs` `rejects_hot_reload_until_gateway_policy_is_revalidated_on_reload`; `src/config/model.rs` strict parsing tests |
...
| T-12 | S, T | A rogue local process binds a loopback backend port, or a compromised trusted tailnet peer serves the configured remote MCP hostname. | **Accepted residual risk, bounded by explicit configuration:** loopback endpoints keep exact fixed `/mcp` paths; remote endpoints require the narrow Tailscale HTTPS profile and an operator-selected machine/tailnet hostname; there is no dynamic backend registration (T-05); backend recovery reconnects transports without retrying tool calls. | `tests/proxy.rs` backend-restart recovery tests; `src/gateway/reconnect.rs`; §4 assumption 2 |
...
## 6. Listener and southbound isolation model

The listener remains loopback-only, while southbound remote access is a
separate explicit trust profile. Enforcement is layered:

1. **Defaults:** `DEFAULT_LISTEN_HOST = "127.0.0.1"`; backend URLs are loopback
   unless the remote-backend policy is explicitly enabled.
2. **Schema validation:** the listener host must equal `127.0.0.1` exactly.
   Backend URLs must be exact loopback `http://127.0.0.1:<port>/mcp`, or—only
   after explicit opt-in—exact default-port `https://<machine>.<tailnet>.ts.net/mcp`.
3. **Policy validation:** `allow_non_loopback_listener` remains rejected.
   `allow_non_loopback_backends` does not permit arbitrary hosts; it activates
   only the constrained Tailscale HTTPS shape from T-02. Hot reload remains off.

Non-loopback **listener** operation still requires a future explicit security
profile. The only supported non-loopback **southbound** operation is the
Tailscale HTTPS profile documented above.
